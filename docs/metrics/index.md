...
| [Cosine Similarity](cosine_similarity.md) | `Metrics/CosineSimilarity/` | aligned score、2-D pairwise matrix、mean/sum reductions |

## 统一约定

- 指标函数接收定义明确的 tensor，不自动执行 argmax、sigmoid、threshold、mask、broadcast 或任务匹配。
- 类别数、类别顺序、ignore/background、zero-division、有效样本与 reduction 策略必须显式固定。
- 区分 aligned metric 与 all-pairs metric；不要平均口径不一致的 batch 比率。
